...
export async function getCategorys() {
    const response = await axios.get("category/rows?_ordering=-id");
    return response.data.data;
  }
  
  export async function getCategory(id) {
    const response = await axios.get("category/rows/" + id);
    return response.data.data[0];
  }
  
  export async function putCategory(category) {
    await axios.put("category/rows/" + category.id, {fields: category});
  }
  
  export async function postCategory(category) {
    await axios.post("category/rows/", {fields: category});
  }
  
  export async function deleteCategory(id) {
    await axios.delete("category/rows/" + id);
  }
